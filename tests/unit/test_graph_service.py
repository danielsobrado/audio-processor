...
class TestGraphService:
    ...
    @pytest.mark.asyncio
    async def test_graph_service_initialization_enabled(self, mock_graph_enabled, mock_neo4j_session):
        """Test GraphService initialization when graph is enabled."""
        with patch('app.db.neo4j_session.get_neo4j_session', return_value=mock_neo4j_session):
...
class TestGraphProcessor:
    ...
    def test_extract_speaker_interactions(self, graph_processor, sample_transcription_result):
        """Test extracting speaker interactions."""
        interactions = graph_processor.extract_speaker_interactions(sample_transcription_result)
        
        assert len(interactions) >= 1
        # Should have interaction between john-doe and jane-smith
        interaction = interactions[0]
        assert set([interaction["from_speaker"], interaction["to_speaker"]]) == {"john-doe", "jane-smith"}

    def test_process_transcription_result(self, graph_processor, sample_transcription_result):
        """Test processing complete transcription result."""
        graph_data = graph_processor.process_transcription_result(sample_transcription_result)
...
class TestGraphServiceIntegration:
    ...
    async def test_process_and_store_transcription_result(
        ...
        with patch('app.db.neo4j_session.get_neo4j_session', return_value=mock_neo4j_session):
            ...
            graph_data = processor.process_transcription_result(sample_transcription_result)
            
            # Store in graph database
            await service.store_conversation_graph(graph_data)
            
            # Verify multiple Neo4j operations were called
            assert mock_neo4j_session.run.call_count >= 4  # speakers, topics, entities, conversation

    @pytest.mark.asyncio
    async def test_graceful_degradation_on_neo4j_error(self, mock_graph_enabled, mock_neo4j_session):
        ...
        with patch('app.db.neo4j_session.get_neo4j_session', return_value=mock_neo4j_session):
            ...
            result = await service.create_speaker_node({"speaker_id": "test", "name": "Test"})
