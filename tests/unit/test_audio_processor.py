...
async def test_process_audio(audio_processor: AudioProcessor):
    ...
    if audio_processor.whisper_model:
        audio_processor.whisper_model.transcribe.return_value = AsyncMock(
            return_value={"segments": [], "language": "en"}
        )
    if audio_processor.diarization_pipeline:
        audio_processor.diarization_pipeline.return_value = AsyncMock(
            return_value="diarization_result"
        )

    with patch(
        "app.core.audio_processor.whisperx.align", return_value={"segments": []}
    ) as mock_align:
        with patch(
            "app.core.audio_processor.whisperx.assign_word_speakers",
            return_value={"segments": []},
        ) as mock_assign_speakers:
...
async def test_initialize_models_success(audio_processor: AudioProcessor):
    ...
    with patch(
        "app.core.audio_processor.whisperx.load_model", return_value=MagicMock()
    ) as mock_load_model:
        with patch(
            "app.core.audio_processor.Pipeline.from_pretrained",
...
async def test_initialize_models_failure(audio_processor: AudioProcessor):
    ...
    with patch(
        "app.core.audio_processor.whisperx.load_model",
