...
class TestUserCRUD:
    ...
    async def test_get_by_email_not_found(self, user_crud, db_session):
        ...
        result = await user_crud.get_by_email(db_session, email="nonexistent@example.com")
        
        assert result is None
        db_session.execute.assert_called_once()

    @pytest.mark.asyncio
    async def test_create_from_token_success(self, user_crud, db_session, token_user_data):
        ...
        with patch.object(User, '__new__', return_value=mock_user):
            result = await user_crud.create_from_token(db_session, token_data=token_user_data)
    ...
    async def test_create_from_token_with_minimal_data(self, user_crud, db_session):
        ...
        # Patch User constructor to return our mock
        with patch.object(User, '__new__', return_value=mock_user):
            result = await user_crud.create_from_token(db_session, token_data=minimal_token_data)
    ...
    async def test_update_user_success(self, user_crud, db_session):
        ...
        result = await user_crud.update(db_session, db_obj=existing_user, obj_in=update_data)
    ...
    async def test_update_user_with_schema_object(self, user_crud, db_session):
        ...
        result = await user_crud.update(db_session, db_obj=existing_user, obj_in=update_schema)
    ...
    async def test_create_regular_user_with_password(self, user_crud, db_session):
        ...
        # Patch User constructor to return our mock
        with patch.object(User, '__new__', return_value=mock_user):
            result = await user_crud.create(db_session, obj_in=user_request, hashed_password=hashed_password)
