...
async def test_llm_factory():
    ...
    try:
        ...
        relationship_extractor = LLMGraphProcessorFactory.create_relationship_extractor(
            settings
        )
        print(
            f"✅ Relationship extractor created: {type(relationship_extractor).__name__}"
        )
...
async def main():
    ...
    if not provider_ok:
        print(
            "❌ OpenRouter provider test failed. Check your API key and configuration."
        )
        return
