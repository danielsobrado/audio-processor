...
class AudioProcessingE2ETest:
    ...
    async def setup(self):
        ...
        if not self.test_audio_files:
            raise FileNotFoundError(f"No supported audio files found in {TEST_AUDIO_DIR}")
        ...
        while time.time() - start_time < max_wait:
            try:
                response = await self.client.get(
                    f"{API_BASE_URL}/api/v1/status/{self.request_id}",
                    headers=headers
                )
    ...
    async def process_task_manually(self, request_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                print(f"   🌐 Running real graph processing...")
                ...
                if graph_result and graph_result.get("success"):
                    ...
                    mock_result["graph_processing"] = {
                        ...
                        "relationships_created": len(graph_result.get("relationships", [])),
                        "sentiment_score": graph_result.get("sentiment", {}).get("score", 0.0),
                        "neo4j_nodes_created": graph_result.get("nodes_created", 0),
                        "neo4j_relationships_created": graph_result.get("relationships_created", 0),
