...
class RealProcessingE2ETest:
    ...
    async def process_audio_file_real(
    ...
    async def validate_real_transcription_results(
        ...
        try:
            ...
            print(
                f"   📝 Transcript length: {len(transcript)} chars, {len(transcript.split())} words"
            )
            ...
            print("   ✅ Real transcription results validated successfully")
            return True
        ...
        try:
            ...
            with driver.session() as session:
                ...
                if not node_results or all(r["count"] == 0 for r in node_results):
                    print("   ⚠️ No recent nodes found in Neo4j")
                    print(
                        "   ℹ️  This might indicate graph processing failed or is disabled"
                    )

                    # Check if graph processing is enabled
                    graph_enabled = (
                        os.getenv("GRAPH_ENABLED", "false").lower() == "true"
                    )
                    if not graph_enabled:
                        print("   ℹ️  Graph processing is disabled in configuration")
                ...
                if graph_count > 0:
                    ...
                    print("   ✅ Real graph processing validation successful")
                    driver.close()
                    return True
                else:
                    print("   ⚠️ No specific graph processing nodes found")
                    print(
                        "   ℹ️  General nodes found, but may not be from LLM processing"
                    )
                    driver.close()
    ...
    def print_comprehensive_summary(
        self, successful_results: list[dict[str, Any]], all_passed: bool
    ):
