...
os.environ.setdefault("GRAPH_ENTITY_EXTRACTION_METHOD", "hybrid")  # regex_patterns, llm_based, hybrid
os.environ.setdefault("GRAPH_TOPIC_EXTRACTION_METHOD", "hybrid")   # keyword_matching, llm_based, hybrid
os.environ.setdefault("GRAPH_SENTIMENT_ANALYSIS_ENABLED", "false")   # true, false
os.environ.setdefault("GRAPH_RELATIONSHIP_EXTRACTION_METHOD", "rule_based")  # rule_based, llm_based, hybrid
        ...
        if settings.graph.entity_extraction_method in ["llm_based", "hybrid"]:
            ...
            try:
                if graph_processor.llm_entity_extractor:
                    ...
                    entities = await graph_processor.llm_entity_extractor.extract_entities(test_text)
        ...
        if settings.graph.topic_extraction_method in ["llm_based", "hybrid"]:
            ...
            try:
                if graph_processor.llm_topic_modeler:
                    ...
                    topics = await graph_processor.llm_topic_modeler.extract_topics(test_text)
        ...
        if settings.graph.sentiment_analysis_enabled:
            ...
            try:
                if graph_processor.llm_sentiment_analyzer:
                    ...
                    sentiment = await graph_processor.llm_sentiment_analyzer.analyze_sentiment(test_text)
...
async def test_sentiment_strategy(graph_data: Dict):
    ...
    try:
        from app.core.processing_strategies import SentimentAnalysisStrategy, ProcessingContext
        
        # Create processing context
        context = ProcessingContext(
            request_data={"request_id": "test-sentiment"}, 
            audio_path=None
        )
        ...
        if not result_context.is_failed() and result_context.processing_result:
            ...
            for i, segment in enumerate(result_context.processing_result["segments"][:3]):
                sentiment = segment.get("sentiment", {})
                print(f"   Segment {i+1}: {sentiment.get('sentiment', 'unknown')} (confidence: {sentiment.get('confidence', 0)})")
        else:
...
async def test_keyword_spotting_strategy(graph_data: Dict):
    ...
    try:
        from app.core.processing_strategies import KeywordSpottingStrategy, ProcessingContext
        
        # Create processing context
        context = ProcessingContext(
            request_data={"request_id": "test-keywords"}, 
            audio_path=None
        )
        context.processing_result = graph_data
        ...
        if not result_context.is_failed() and result_context.processing_result:
            ...
            for i, segment in enumerate(result_context.processing_result["segments"][:3]):
                ...
                print(f"   Segment {i+1}: {len(entities)} entities, {len(topics)} topics")
        else:
