...
class TestTranscriptionAPITranslation:
    ...
    def test_transcribe_translation_missing_target_language(
        self, mock_get_settings, mock_get_user_id
    ):
    ...
    @patch("app.api.v1.endpoints.transcribe.get_current_user_id")
    @patch("app.api.v1.endpoints.transcribe.get_settings_dependency")
    def test_transcribe_translation_disabled(self, mock_get_settings, mock_get_user_id):
    ...
    def test_transcribe_no_translation_params(self):
        ...
        with (
            patch(
                "app.api.v1.endpoints.transcribe.get_current_user_id"
            ) as mock_get_user_id,
            patch(
                "app.api.v1.endpoints.transcribe.get_settings_dependency"
            ) as mock_get_settings,
            patch(
                "app.api.v1.endpoints.transcribe.get_job_queue"
            ) as mock_get_job_queue,
            patch(
                "app.api.v1.endpoints.transcribe.get_transcription_service"
            ) as mock_get_transcription_service,
            patch(
