    ...
    "segments": [
        {
            "start": 0.0,
            "end": 4.8,
            "text": "Good morning team, this is John Smith, project manager. Let's review our AI budget.",
            "speaker": "john-smith",
            "confidence": 0.95,
        },
        {
            "start": 4.8,
            "end": 11.2,
            "text": "Hi John, Sarah here from finance. Our Q1 budget is $500,000. Email me at sarah@company.com for details.",
            "speaker": "sarah-jones",
            "confidence": 0.92,
        },
        {
            "start": 11.2,
            "end": 17.9,
            "text": "Thanks Sarah! Mike, can you update us on the machine learning algorithms? Deadline is March 15th.",
            "speaker": "john-smith",
            "confidence": 0.88,
        },
        {
            "start": 17.9,
            "end": 25.3,
            "text": "Sure John. The neural networks need GPU clusters. Technical specs at mike.wilson@company.com",
            "speaker": "mike-wilson",
            "confidence": 0.91,
        },
        {
            "start": 25.3,
            "end": 30.5,
            "text": "Perfect! Next meeting Friday 2:00 PM. Call me at 555-123-4567 for urgent issues.",
            "speaker": "john-smith",
            "confidence": 0.93,
        },
    ],
...
async def cleanup_previous_demo(manager, conversation_id: str):
    ...
    print(
        f"🧹 Cleaning up previous demo data for conversation ID: {conversation_id}..."
    )
    query = """
...
async def main():
    ...
    if not settings.graph.enabled:
        ...
        print(
            "Please set GRAPH_ENABLED=true in your .env file or enable it in your YAML config and restart."
        )
        return
    ...
    try:
        ...
        print(
            f"   MATCH (c:Conversation {{id: '{conversation_id}'}})-[r]-(n) RETURN c, r, n"
        )
        print()

        print("👥 2. View Speakers and their Segments:")
        print(
            f"   MATCH (sp:Speaker)-[:SPEAKS_IN]->(c:Conversation {{id: '{conversation_id}'}})<-[:CONTAINS]-(c)<-[:CONTAINS]-(seg:TranscriptSegment {{speaker_id: sp.id}}) RETURN sp, seg"
        )
        print()

        print("🏷️  3. View Topics Discussed by Speakers:")
        print(
            f"   MATCH (sp:Speaker)-[d:DISCUSSES]->(t:Topic) WHERE (sp)-[:SPEAKS_IN]->(:Conversation {{id: '{conversation_id}'}}) RETURN sp, d, t"
        )
        print()

        print("📧 4. View Entities Mentioned in Segments:")
        print(
            f"   MATCH (e:Entity)<-[m:MENTIONS]-(seg:TranscriptSegment)-[:CONTAINS]-(:Conversation {{id: '{conversation_id}'}}) RETURN e, m, seg"
        )
        print()

        print("➡️ 5. View the Sequential Flow of the Conversation:")
        print(
            f"   MATCH p=(:Conversation {{id: '{conversation_id}'}})-[:CONTAINS]->(seg1:TranscriptSegment)-[:FOLLOWS*]->(seg2:TranscriptSegment) RETURN p"
        )
        print()

        print("=" * 60)
        print(
            "🎯 In the Neo4j Browser, results will appear as a table. Click the 'Graph' icon on the left to visualize."
        )
        print("=" * 60)
