...
"""
        ...
        try:
            ...
            for topic in topics_data:
                topics.append((topic["topic_name"], topic.get("confidence", 0.7)))
...
class LLMGraphProcessorFactory:
    ...
    def create_llm_provider(settings) -> LLMProvider:
        ...
        if provider == "openai":
            if not api_key:
                raise ValueError(
                    "OpenAI API key is required but not found in settings or environment"
                )
            ...
            if not api_key:
                raise ValueError(
                    "Anthropic API key is required but not found in settings or environment"
                )
            return AnthropicProvider(api_key=api_key, model=settings.graph.llm_model)
        elif provider == "local":
            ...
            if not api_key:
                raise ValueError(
                    "OpenRouter API key is required but not found in settings or environment"
                )
            return OpenRouterProvider(api_key=api_key, model=settings.graph.llm_model)
        else:
