...
class DatabaseSettings(BaseSettings):
    ...
    url: str = Field(
        default="postgresql://localhost/audio_processor_dev", alias="DATABASE_URL"
    )
...
class AuthSettings(BaseSettings):
    ...
    keycloak_url: str = Field(
        default="http://localhost:8080/auth", alias="KEYCLOAK_URL"
    )
...
class WhisperXSettings(BaseSettings):
    ...
    return_char_alignments: bool = Field(
        default=False, alias="WHISPERX_CHAR_ALIGNMENTS"
    )
...
class TranslationSettings(BaseSettings):
    ...
    model_name: str = Field(
        default="Helsinki-NLP/opus-mt-en-es", alias="TRANSLATION_MODEL_NAME"
    )
...
class GraphDatabaseSettings(BaseSettings):
    ...
    type: str = Field(
        default="neo4j", alias="GRAPH_DATABASE_TYPE"
    )  # neo4j, arangodb, etc.
...
class Settings(BaseSettings):
    ...
    max_file_size: int = Field(
        default=MAX_UPLOAD_FILE_SIZE_BYTES, alias="MAX_FILE_SIZE"
    )
