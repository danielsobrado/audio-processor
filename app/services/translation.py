...
class TranslationService:
    ...
    async def initialize_model(self):
        ...
        try:
            ...
            self.pipeline = pipeline(
                "translation", model=self.model_name, device=self.device
            )
    ...
    async def translate_text(
        ...
        if not self.pipeline:
            logger.warning(
                "Translation pipeline not available. Returning original text."
            )
            return text
        ...
        try:
            ...
            logger.error(
                f"Translation to '{target_language}' failed: {e}", exc_info=True
            )
            return (
                text  # Return original text on failure to avoid breaking the entire job
            )
