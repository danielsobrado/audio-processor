...
class GraphService:
    ...
    async def get_conversation_graph(self, conversation_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await manager.execute_read_transaction(query, {"conversation_id": conversation_id})
            
            if not result:
                return {"conversation_id": conversation_id, "nodes": [], "edges": [], "found": False}
    ...
    async def get_speaker_network(self, conversation_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = await manager.execute_read_transaction(query, {"conversation_id": conversation_id})
        ...
        try:
            ...
            for result in results:
                relationships.append({
                    "source_node": {
                        "id": result["n"].get("id"),
                        "type": list(result["n"].labels)[0] if result["n"].labels else "Unknown",
                        "properties": dict(result["n"])
                    },
                    "target_node": {
                        "id": result["related"].get("id"),
                        "type": list(result["related"].labels)[0] if result["related"].labels else "Unknown",
    ...
    async def find_shortest_path(
        ...
        try:
            ...
            results = await manager.execute_read_transaction(query, {
